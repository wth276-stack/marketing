...
新增：
- **AI 揀互動目標顯示**：`r.interactionGoal` 有值時喺腳本區上方顯示 read-only「AI 揀嘅互動目標：{interactionGoal}」。
...
- 喺 `#script-review` 渲染 `r.scriptReview`：
  ...
  - 修正版腳本：`<textarea id="polished-script">` 顯示 `polishedScript` +「用修正版腳本」掣（`#use-polished-script`）。
